...
print(features)  # From the save_object in your training code
```
...
# Save Final Tuned Random Forest Model for Shiny

rf_model_final <- rf_tuned$finalModel  # Your final model

# Create a list with the model and additional metadata
save_object <- list(
  model = rf_model_final,  # The actual model
  feature_names = c("IndicatorType", "DenominatorWeighted", "DenominatorUnweighted", 
                    "CharacteristicCategory", "PopulationGroup", "SurveyYear", "IndicatorCategoryHigh"),  # All features
  levels = list(
    IndicatorCategoryHigh = levels(model_data$IndicatorCategoryHigh),  # Levels for categorical variables
    IndicatorType = levels(model_data$IndicatorType),
    CharacteristicCategory = levels(model_data$CharacteristicCategory),
    PopulationGroup = levels(model_data$PopulationGroup)
...
saveRDS(save_object, file = "rf_nutrition_model.rds")
cat("Random Forest model saved successfully as rf_nutrition_model.rds")
```
