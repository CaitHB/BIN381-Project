...
# Columns to keep
keep_cols <- c(
  "SurveyYear", "IndicatorId", "Indicator", "IndicatorType", "Value", "Value_zscore",
...
```{r}
# Compute weighted scores and save
merged_data <- raw_merged_data %>%
  mutate(
    IndicatorCategoryHigh = case_when(
      ...
      str_detect(IndicatorId, "^AN_NUTS_W_TH|^AN_NUTS_W_OWT|^AN_NUTS_W_OBS|
                          ^AN_NUTS_M_TH|^AN_NUTS_M_OWT|^AN_NUTS_M_OBS") ~ "Adult Nutrition Status",
  ...
  group_by(SurveyYear, PopulationGroup) %>%
  mutate(
    ChildNutritionScore = if_else(
      PopulationGroup == "Children",
      sum(
        Value[IndicatorCategoryHigh %in% c(
          "Child Growth (Height for Age)",
          "Child Growth (Weight for Age)",
          "Child Growth (Weight for Height)"
        )] *
        DenominatorWeighted[IndicatorCategoryHigh %in% c(
          "Child Growth (Height for Age)",
          "Child Growth (Weight for Age)",
          "Child Growth (Weight for Height)"
        )],
    ...
    AdultBMIScore = if_else(
      PopulationGroup == "Adults",
      sum(
        Value[IndicatorCategoryHigh == "Adult BMI"] *
        DenominatorWeighted[IndicatorCategoryHigh == "Adult BMI"],
        na.rm = TRUE
      ) /
      sum(
        DenominatorWeighted[IndicatorCategoryHigh == "Adult BMI"],
