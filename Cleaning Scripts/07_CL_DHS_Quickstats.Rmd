...
data.frame(
  Column = names(dhs_df),
  Missing_Percentage = paste0(round(colMeans(is.na(dhs_df)) * 100, 2), "%")
  )
...
data.frame(
  Column = names(dhs_df),
  Missing_Data = paste0(colSums(is.na(dhs_df)))
  )
```
...
dhs_df <- dhs_df %>%
  mutate(
    DenominatorUnweighted = if_else(DataId == "46433", 8514, DenominatorUnweighted),
    DenominatorUnweighted   = if_else(DataId == "563776", 8514, DenominatorUnweighted)
  )

```

```{r}
ggplot(dhs_df, aes(x = DenominatorWeighted, y = DenominatorUnweighted)) +
...
ggplot(dhs_df, aes(y = DenominatorWeighted)) +
...
Q1_w <- quantile(dhs_df$DenominatorWeighted, 0.25, na.rm = TRUE)
Q3_w <- quantile(dhs_df$DenominatorWeighted, 0.75, na.rm = TRUE)
IQR_w <- Q3_w - Q1_w
lower_w <- Q1_w - 1.5 * IQR_w
upper_w <- Q3_w + 1.5 * IQR_w

Q1_uw <- quantile(dhs_df$DenominatorUnweighted, 0.25, na.rm = TRUE)
Q3_uw <- quantile(dhs_df$DenominatorUnweighted, 0.75, na.rm = TRUE)
IQR_uw <- Q3_uw - Q1_uw
...
#save cleaned data
```{r}
write_csv(dhs_df, here("cleaned_data", "dhs-quickstats_cleaned.csv"))
```